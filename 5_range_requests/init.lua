local space_size = 30000000
local space_engine = 'memtx'
...
--    {name = 'non_unique', type = 'unsigned'},
--    {name = 'long_step', type = 'unsigned'},
--    {name = 'repeating', type = 'unsigned'},
--    {name = 'unique', type = 'unsigned'},
}
local field_by_name = {}
for _, field in pairs(format) do
...
local function long_step(step_count)
    return function(i)
        if i == -1 then
            return 'long, steps: ' .. step_count
        end
        return math.floor(i / step_count)
    end
end
local function repeating(steps_to_repeat)
    return function(i)
        if i == -1 then
            return 'repeating, steps: ' .. steps_to_repeat
        end
        return i % steps_to_repeat
    end
end
...
local gen_field_value = {
    incrementing,
--    incrementing,   -- 123456789
--    random(1, 100), -- 887241132
--    long_step(100), -- 111222333
--    repeating(100), -- 123123123
--    random_unique,  -- 975642138
}
...
local function clear()
    for _, file in pairs(fio.glob('./000*')) do
        fio.unlink(file)
    end
end
...
for _, part in pairs(search_index.parts) do
    ...
    table.insert(c_parts, part.fieldno - 1)
    table.insert(c_parts, part.type)
end
